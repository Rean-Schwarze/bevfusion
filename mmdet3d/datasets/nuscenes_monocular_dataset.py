...
class NuScenesMultiViewDataset(MultiViewMixin, NuScenesDataset):
    ...
    def show(self, results, out_dir='trash', bev_seg_results=None, thr=0.3, fps=3):
        ...
        colors = get_colors()
        batch_size = 600
        temp_batch_dir = os.path.join(out_dir, 'temp_batches')
        ...
        for batch_idx in range(num_batches):
            ...
            print(f'处理批次 {batch_idx + 1}/{num_batches} ({start_idx}-{end_idx - 1})')

            batch_img_gt, batch_img_pred, batch_bev_gt, batch_bev_pred = [], [], [], []
            for i, result in enumerate(batch_results):
                info = self.get_data_info(i)
                gt_bboxes = self.get_ann_info(i)
                print('batch {}: saving image {}/{} to {}'.format(batch_idx, start_idx + i, len(results), out_dir))
                # draw 3d box in BEV
                scale_fac = 10
                out_file_dir = str(start_idx + i)
                ###### draw BEV pred ######
                bev_pred_img = np.zeros((100 * scale_fac, 100 * scale_fac, 3))
                if bev_seg_results is not None:
                    bev_pred_road, bev_pred_lane = bev_seg_results[i]['seg_pred_road'], bev_seg_results[i][
                        'seg_pred_lane']
                ...
                batch_bev_gt.append(mmcv.imrescale(bev_gt_img, 0.5))
                batch_bev_pred.append(mmcv.imrescale(bev_pred_img, 0.5))
                ###### draw BEV pred ######
                ...
                batch_img_pred.append(tmp_img_pred)
                ...
                batch_img_gt.append(tmp_img_gt)
                ###### generate videos step:1 ######

            # 为当前批次生成视频片段
            batch_pred_video = os.path.join(temp_batch_dir, f'pred_batch_{batch_idx}.mp4')
            batch_gt_video = os.path.join(temp_batch_dir, f'gt_batch_{batch_idx}.mp4')

            gen_video(batch_img_pred, batch_bev_pred, temp_batch_dir, batch_idx, 'pred', fps=fps)
            gen_video(batch_img_gt, batch_bev_gt, temp_batch_dir, batch_idx, 'gt', fps=fps)

            # 释放当前批次的内存
            del batch_img_gt, batch_img_pred, batch_bev_gt, batch_bev_pred
        ###### generate videos step:2 ######
        # 合并所有视频片段
        print("合并所有视频片段...")
        merge_videos(temp_batch_dir, out_dir, 'pred', num_batches)
        merge_videos(temp_batch_dir, out_dir, 'gt', num_batches)
...
def merge_videos(temp_dir, out_dir, prefix, num_batches):
    """合并多个视频片段为一个完整视频"""
    # 这里需要实现视频合并的逻辑
    # 可以使用OpenCV、moviepy或ffmpeg-python等库
    ...
    for i in range(num_batches):
        clip_path = os.path.join(temp_dir, f'video_{prefix}_batch_{i}.mp4')
        if os.path.exists(clip_path):
            clips.append(VideoFileClip(clip_path))

    if clips:
        final_clip = concatenate_videoclips(clips)
        final_clip.write_videofile(os.path.join("/root/autodl-tmp/Fast-BEV/output", f'video_{prefix}.mp4'), fps=30)
        # 释放内存
        for clip in clips:
            clip.close()
        final_clip.close()
...
def gen_video(img_list, bev_list, out_dir, batch_idx, mode='pred', fps=3):
    if mode == 'pred':
        out_video_path = os.path.join(out_dir, f'video_pred_batch_{batch_idx}.mp4')
    else:
        assert mode == 'gt'
        out_video_path = os.path.join(out_dir, f'video_gt_batch_{batch_idx}.mp4')
    tmp_list = []
