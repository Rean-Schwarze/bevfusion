...
__all__ = ["RelationResponseDistiller", "BEVResponseDistillerFusedLRC2CMask", "BEVResponseDistillerFusedLRC2CMaskScale",
           "BEVResponseDistillerFusedLRC2CMaskScaleRelation", "BEVResponseDistillerFusedLRC2CMaskScaleGaussian",
           "BEVResponseDistillerFusedLRC2CMaskScaleRelationGaussian"]
...
class RelationResponseDistiller(BEVFusion):
    def __init__(
            ...
            student_ckpt_path,
            # For fused kd
            fused_affinity_loss=dict(type="Affinity_Loss", reduction="mean", loss_weight=1.0,
                                     downsample_size=[32, 16, 8], input_channels=256, use_adapt=True),
        ...
        self.use_depth_loss_student = ((encoders_student.get('camera', {}) or {}).get('vtransform', {}) or {}).get(
            'type', '') in ['BEVDepth', 'AwareBEVDepth', 'DBEVDepth', 'AwareDBEVDepth']
    ...
    def init_weights_distill(self, teacher_ckpt_path, student_ckpt_path) -> None:
        ...
        # encoders_student
        student_encoder_ckpt = _load_checkpoint_with_prefix(prefix='encoders', filename=student_ckpt_path,
                                                            map_location='cpu')
        load_state_dict(self.encoders_student, student_encoder_ckpt)
        # fuser_student
        student_fuser_ckpt = _load_checkpoint_with_prefix(prefix='fuser', filename=student_ckpt_path,
                                                          map_location='cpu')
        load_state_dict(self.fuser_student, student_fuser_ckpt)
        # decoder_student
        student_decoder_ckpt = _load_checkpoint_with_prefix(prefix='decoder', filename=student_ckpt_path,
                                                            map_location='cpu')
        load_state_dict(self.decoder_student, student_decoder_ckpt)
        # heads_student
        student_heads_ckpt = _load_checkpoint_with_prefix(prefix='heads', filename=student_ckpt_path,
                                                          map_location='cpu')
        ...
        # encoders
        teacher_encoder_ckpt = _load_checkpoint_with_prefix(prefix='encoders', filename=teacher_ckpt_path,
                                                            map_location='cpu')
        ...
        self.encoders.eval()
        # fuser
        teacher_fuser_ckpt = _load_checkpoint_with_prefix(prefix='fuser', filename=teacher_ckpt_path,
                                                          map_location='cpu')
        ...
        self.fuser.eval()
        # decoder
        teacher_decoder_ckpt = _load_checkpoint_with_prefix(prefix='decoder', filename=teacher_ckpt_path,
                                                            map_location='cpu')
        ...
        self.decoder.eval()
        # heads
        teacher_heads_ckpt = _load_checkpoint_with_prefix(prefix='heads', filename=teacher_ckpt_path,
                                                          map_location='cpu')
    ...
    def forward_single(
        ...
        if self.training:
            ...
            outputs["loss/object/fused_affinity_kd"] = self.fused_affinity_loss_scale_kd * self.fused_affinity_loss(
                fused_student_features, fused_teacher_features)
    ...
    def forward_single(
        ...
        if self.training:
            ...
            if self.fused_loss is not None:
                outputs["loss/object/fused_feat_kd"] = self.fused_feat_loss_scale_kd * self.fused_feat_loss(
                    fused_student_features, fused_teacher_features)
            # Cam2Cam KD loss
            if self.cam_feat_loss_scale_kd is not None:
                ...
                outputs["loss/object/cam_mask_feat_kd"] = self.cam_feat_loss_scale_kd * self.cam_feat_loss(cam_student,
                                                                                                           cam_teacher,
                                                                                                           gt_mask_feat)
...
class BEVResponseDistillerFusedLRC2CMaskScale(BEVFusion):
    ...
    def forward_single(
        ...
        if self.training:
            ...
            # Fusion KD loss
            if self.fused_loss is not None:
                outputs["loss/object/fused_mask_scale_feat_kd"] = self.fused_feat_loss_scale_kd * self.fused_feat_loss(
                    fused_student_features, fused_teacher_features, gt_mask_feat)
            # Cam2Cam KD loss
            if self.cam_feat_loss_scale_kd is not None:
                # print("cam student: ", cam_student)
                # print("cam teacher: ", cam_teacher)
                outputs["loss/object/cam_mask_scale_feat_kd"] = self.cam_feat_loss_scale_kd * self.cam_feat_loss(
                    cam_student, cam_teacher, gt_mask_feat)
    ...
    def forward_single(
        ...
        if self.training:
            ...
            if self.use_gaussian:
                ...
                if self.fused_loss is not None:
                    fused_feat_loss_gaussian = self.fused_feat_loss_scale_kd * self.fused_feat_loss(
                        fused_student_features, fused_teacher_features)  # (B, H, W)
                    # mean by batchsize and map size
                    fused_feat_loss_gaussian = torch.sum(fused_feat_loss_gaussian * gaussian_fg_map) / torch.sum(
                        gaussian_fg_map)
                    outputs["loss/object/fused_gaussian_mask_scale_feat_kd"] = fused_feat_loss_gaussian
                # Cam2Cam KD loss
                if self.cam_feat_loss_scale_kd is not None:
                    cam_feat_loss_gaussian = self.cam_feat_loss_scale_kd * self.cam_feat_loss(cam_student,
                                                                                              cam_teacher)  # (B, H, W)
                    # mean by batchsize and map size
                    cam_feat_loss_gaussian = torch.sum(cam_feat_loss_gaussian * gaussian_fg_map) / torch.sum(
                        gaussian_fg_map)
                ...
                if self.fused_loss is not None:
                    outputs[
                        "loss/object/fused_mask_scale_feat_kd"] = self.fused_feat_loss_scale_kd * self.fused_feat_loss(
                        fused_student_features, fused_teacher_features, gt_mask_feat)
                # Cam2Cam KD loss
                if self.cam_feat_loss_scale_kd is not None:
                    outputs["loss/object/cam_mask_scale_feat_kd"] = self.cam_feat_loss_scale_kd * self.cam_feat_loss(
...
class BEVResponseDistillerFusedLRC2CMaskScaleRelation(BEVFusion):
    def __init__(
            ...
            cam_domain_adaptation: Dict[str, Any] = None,
            cam_conv_channel: Dict[str, Any] = None,
            cam_mse_loss: Dict[str, Any] = None,
            cam_feat_mse_loss_scale_kd: float = 1.0,
            cam_loss: Dict[str, Any] = None,
            cam_feat_loss_scale_kd: float = 1.0,
            fused_domain_adaptation: Dict[str, Any] = None,
            fused_conv_channel: Dict[str, Any] = None,
            ...
            # fused_affinity_loss = dict(type="Affinity_Loss", reduction="mean", loss_weight=1.0, downsample_size=[32, 16, 8, 4], input_channels=256, use_adapt=True),
            fused_affinity_loss_scale_kd: float = 1.0,
            **kwargs,
    ) -> None:
        super().__init__(encoders, fuser, decoder, heads)
        ...
        if fused_loss is not None:
            self.fused_feat_loss = build_loss(fused_loss)
        if cam_mse_loss is not None:
            self.cam_mse_loss = build_loss(cam_mse_loss)
        if cam_loss is not None:
            self.cam_feat_loss = build_loss(cam_loss)
        ...
        # Cam2Cam KD loss scale
        self.cam_feat_mse_loss_scale_kd = cam_feat_mse_loss_scale_kd
        self.cam_feat_loss_scale_kd = cam_feat_loss_scale_kd
    ...
    def extract_camera_features_student(
        ...
        B, N, C, H, W = x.size()  # batchsize, num_view, channel, height, width
        # print("x.size:",x.size()) # [2?, 6, 3, 256, 704]
        x = x.view(B * N, C, H, W)  # batchsize * num_view, channel, height, width
...
#             print("before lidar voxelize_student")
            feats, coords, sizes = self.voxelize_student(x, sensor)
    ...
    def forward_single(
        ...
        if self.training:
            # x_teacher, lidar_teacher, cam_teacher = self.get_gated_feature_teacher(img,
            x_teacher, cam_teacher, lidar_teacher = self.get_fused_feature_teacher(img,
                                                                                   points,
                                                                                   camera2ego,
                                                                                   lidar2ego,
                                                                                   lidar2camera,
                                                                                   ...
                                                                                   depths)
            fused_teacher_features = x_teacher
            # print("teacher feature shape: ", fused_teacher_features.shape) # (3(←batch_size), 256, 180, 180)
            x_teacher = self.decoder["backbone"](x_teacher)
            x_teacher = self.decoder["neck"](x_teacher)
        ...
        if self.fuser_student is not None:
            # _, x_student, cam_student, radar_student = self.fuser_student(features_student)
            x_student = self.fuser_student(features_student)
            fused_student_features = x_student
            # print("fused student feature shape: ", fused_student_features.shape)  # (3, 64, 128, 128)
        ...
        if self.training:
            ...
            for type, head in self.heads_student.items():
                if type == "object":  # for 3d object detection
                    pred_dict = head(x_student, metas)
                    pred_dict_det = pred_dict
                    losses_student = head.loss(gt_bboxes_3d, gt_labels_3d, pred_dict_det)

                    # Teacher Model Forward
                    pred_dict_kd = pred_dict
                    pred_dict_teacher = self.heads[type](x_teacher, metas)
                    losses_teacher = self.heads[type].loss(gt_bboxes_3d, gt_labels_3d, pred_dict_teacher)
                    ...
                    if self.lr_kd_loss is not None:
                        ...
                        lidar_heatmap_dynamic = torch.mean(lidar_heatmap_dynamic, dim=1, keepdim=True)  # 2, 1, 180, 180
                    # print('----------------------')
                    losses_response_kd = head.loss_soft(pred_dict_teacher, pred_dict_kd, losses_teacher, losses_student)

                elif type == "map":  # for 3d semantic segmentation
                    losses_student = head(x_student, gt_masks_bev)
                else:
                    raise ValueError(f"unsupported head: {type}")

                # Det Loss scale (DET)
                for name, val in losses_student.items():
                    if val.requires_grad:
                        outputs[f"loss/{type}/{name}"] = val * self.loss_scale_student[type]
                    else:
                        outputs[f"stats/{type}/{name}"] = val
            ...
            if self.fused_affinity_loss is not None:
                # Fusion Relation KD loss (RELD)
                assert fused_student_features.shape == fused_teacher_features.shape, \
                    f"Dimension mismatch: student {fused_student_features.shape} vs teacher {fused_teacher_features.shape}"
                fused_student_features = self.normalize_feature(fused_student_features)
                fused_teacher_features = self.normalize_feature(fused_teacher_features)
                fused_student_features = self.check_nan_inf(fused_student_features)
                fused_teacher_features = self.check_nan_inf(fused_teacher_features)
                outputs[
                    "loss/object/fused_affinity_kd"] = self.fused_affinity_loss_scale_kd * self.fused_feat_affinity_loss(
                    fused_student_features, fused_teacher_features)

            # Cam2Cam KD loss
            if self.cam_mse_loss is not None:
                outputs["loss/object/cam_mse_feat_kd"] = (self.cam_feat_loss_scale_kd *
                                                          self.cam_mse_loss(cam_student, cam_teacher))
