...
__all__ = ['yolo_v11_n', 'yolo_v11_t', 'yolo_v11_s', 'yolo_v11_m', 'yolo_v11_l', 'yolo_v11_x']
...
class Head(torch.nn.Module):
    ...
    def __init__(self, nc=80, filters=(), head_feature_only=True):
        super().__init__()
        ...
        self.head_feature_only = head_feature_only
    ...
    def forward(self, x):
        ...
        if self.training or self.head_feature_only:
            return x
...
class YOLO(torch.nn.Module):
    def __init__(self, width, depth, csp, num_classes, head_feature_only=True, pretrained=None, init_cfg=None):
        super().__init__()
        ...
        self.head = Head(num_classes, (width[3], width[4], width[5]), head_feature_only)
    ...
    def init_weights(self, pretrained=None):
        ...
        if self.init_cfg is None and pretrained is None:
            ...
            # if missing_keys:
            #     logger.warning(f"YOLO v11_init_weight\nMissing keys: {missing_keys}")
            # if unexpected_keys:
            #     logger.warning(f"YOLO v11_init_weight\nUnexpected keys: {unexpected_keys}")


@BACKBONES.register_module()
class yolo_v11_n(YOLO):
    def __init__(self, num_classes: int = 80, **kwargs):
        csp = [False, True]
        depth = [1, 1, 1, 1, 1, 1]
        width = [3, 16, 32, 64, 128, 256]
        super().__init__(width, depth, csp, num_classes, **kwargs)


@BACKBONES.register_module()
class yolo_v11_t(YOLO):
    def __init__(self, num_classes: int = 80, **kwargs):
        csp = [False, True]
        depth = [1, 1, 1, 1, 1, 1]
