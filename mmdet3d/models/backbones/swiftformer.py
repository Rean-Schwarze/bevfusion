...
__all__ = ["SwiftFormer_XS", "SwiftFormer_S", "SwiftFormer_L1", "SwiftFormer_L3"]
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None,
        ...
        if isinstance(act_layer, str):
            self.act = activation_mapping.get(act_layer)
...
class SwiftFormerEncoder(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4.,
        ...
        # print(f"\n\nType of dim: {type(dim)}, Value of dim: {dim}\n\n")
...
class SwiftFormer_XS(SwiftFormer):
    def __init__(self, pretrained=False, **kwargs):
        ...
        super().__init__(layers=layers, embed_dims=embed_dims, downsamples=downsamples,
                         pretrained=pretrained, **kwargs)
        self.default_cfg = self._cfg(crop_pct=0.9)

    def _cfg(self, crop_pct):
        return {'crop_pct': crop_pct}
