...
class Affinity_Loss(nn.Module):
    def __init__(self, reduction="mean", loss_weight=1.0, downsample_size=[32, 16, 8, 4], input_channels=256,
                 use_adapt=True):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor):
        ...
        assert C_in == C_ta, f"Channels mismatch: {C_in} vs {C_ta}"
        input_ds_lst = []
        target_ds_lst = []
        for i in range(len(self.downsample_size)):
            ...
            # 归一化特征（L2范数）
            input_ds_temp = F.normalize(input_ds_temp, p=2, dim=1)
            target_ds_temp = F.normalize(target_ds_temp, p=2, dim=1)

            if self.use_adapt:
                input_ds_temp = self.adapt_modules[i](input_ds_temp)

            input_ds_temp = input_ds_temp.reshape(B, C_in, -1)  # B, C_in, H*W
            input_affinity = torch.bmm(input_ds_temp.permute(0, 2, 1),
                                       input_ds_temp)  # (B, H*W, C_in) * (B, C_in, H*W) = (B, H*W, H*W)

            target_ds_temp = target_ds_temp.reshape(B, C_ta, -1)  # B, C_ta, H*W
            target_affinity = torch.bmm(target_ds_temp.permute(0, 2, 1),
                                        target_ds_temp)  # (B, H*W, C_ta) * (B, C_ta, H*W) = (B, H*W, H*W)

            feature_ditill_loss += F.l1_loss(input_affinity, target_affinity,
                                             reduction=self.reduction) / B * self.loss_weight
